...
public class AuditController extends AbstractController {
    ...
    private static final String AUDIT_CREATE    =   "/audit/create";

    @ResponseStatus(value = HttpStatus.OK)
    @RequestMapping(value = AUDIT_CREATE, method = RequestMethod.POST)
    @Produces(MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody HttpEntity<APIResponse> create (@RequestBody AuditRecord auditRecord) throws Exception {
        HttpEntity<APIResponse> apiResponse = null;
        logger.info("Entered");
        try {
            getAuditService().create(auditRecord); 
    ...
    private AuditService getAuditService () {
        return APIContextAware.getAuditService();
    }
}
